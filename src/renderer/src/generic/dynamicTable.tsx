...
const Capitalization = ({
	...
	const constantsArray =
		columnProps?.refArray ||
		columns?.find((c) => c?.key === columnProps?.index)?.options;
			...
			? new Date(
					new Date().setFullYear(
						new Date().getFullYear() +
							parseInt(values?.[columnProps?.duration] || 0) -
							1,
					),
				)
	...
	const start =
		(columnProps?.duration ? new Date() : values?.[columnProps?.start]) ||
		columnProps?.start;
	...
	const index = constantsArray?.findIndex(
		(e) =>
			e?.value ===
			parseFloat(
				values?.[columnProps?.index] || formValues?.[columnProps?.index],
			),
	);
				...
				<div>
					<math>
						...
						<CoefficientInfo
							...
							startIndex={columnProps?.displayStartIndex}
						>
							<mn>{coef?.value}</mn>
						</CoefficientInfo>
					</math>
	...
	return (
		<Money
			value={(
				parseFloat(values?.[columnProps?.amount] || values?.amount || 0) *
				(coef?.value || 0)
			).toFixed(2)}
						...
						return (
							<tr key={field.id}>
								{columns.map((column, colIndex) => {
									...
									const error = errors?.[base + "." + fieldName];
									...
									return (
										...
										>
											...
											{column?.additionalContent &&
												column?.additionalContent(rowData)}
										</ErrorWrapper>
									);
								})}
