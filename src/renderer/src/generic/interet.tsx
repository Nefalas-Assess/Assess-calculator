...
const getDaysPerYearInRange = (startDate, endDate) => {
	...
	if (endYear.getFullYear()?.toString()?.length !== 4) {
		return result;
	}
	...
	for (
		...
		const adjustedInterval = {
			start: isWithinInterval(startDate, yearInterval)
				? startDate
				: currentYearStart,
		...
		const daysInYear =
			differenceInCalendarDays(adjustedInterval.end, adjustedInterval.start) +
			1;
...
const getAmountForYear = (year) => {
	...
	const found = rates.find(
		(entry) => yearStr >= entry.start && yearStr <= entry.end,
	);
		...
		return (
			<div>
				<div>
					...
					>
						{start && isValid(new Date(start)) && format(start, "dd/MM/yyyy")}{" "}
						<FaArrowRightLong />{" "}
