...
const ITMenagereForm = ({ initialValues, onSubmit, editable = true }) => {
  ...
  const getTotalAmount = useCallback(
    (item, days, child) => {
      ...
      const baseAmount = parseFloat(amount) + childAmount * indicativePersonChargeAmount
      return (
        (parseInt(days) || 0) *
    ...
    [data, indicativePersonChargeAmount]
  )
  ...
  const renderTooltipTotal = useCallback(
    (item, days, child) => {
      const { amount = 0, percentage = 0, contribution = 0 } = item
      const childAmount = child?.reduce((acc, value) => {
        const percentage = parseFloat(value?.days?.percentageBefore25 || 0)
        ...
        <div>
          <div>
            <math>
              ...
              <mn>{childAmount}</mn>
              <mo>x</mo>
              <mn>{indicativePersonChargeAmount}</mn>
              <mo>)</mo>
              <mo>=</mo>
              <mn>{baseAmount}</mn>
            </math>
  ...
  const columns = [
    ...
    { header: 'common.days', key: 'days', type: 'calculated' },
    {
      ...
      key: 'children',
      render: (e, rowData) => <ChildrenCell>{getChildOnPeriod(rowData)}</ChildrenCell>
    },
    {
        ...
        <div style={{ fontSize: 12 }}>
          ...
          {(
            getChildOnPeriod(rowData)?.reduce((acc, value) => {
              const percentage = parseFloat(value?.days?.percentageBefore25 || 0)
              return acc + percentage
            }, 0) * indicativePersonChargeAmount
          )?.toFixed(2)}
          €)
        </div>
      )
