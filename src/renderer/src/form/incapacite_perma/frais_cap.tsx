...
const FraisCapForm = ({ initialValues, onSubmit, editable = true }) => {
	...
	useEffect(() => {
		const valuesChanged =
			...
			JSON.stringify(chargesValues) !==
				JSON.stringify(previousValuesRef.current?.charges) ||
			JSON.stringify(paidValues) !==
				JSON.stringify(previousValuesRef.current?.paid) ||
			JSON.stringify(packageValues) !==
				JSON.stringify(previousValuesRef.current?.package);
