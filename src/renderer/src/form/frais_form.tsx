...
export const FraisForm = ({ onSubmit, initialValues, editable = true }) => {
	...
	const totalDeplacementFrais = useMemo(
		() => ({
			value: (
				formValues?.deplacement_value * formValues?.deplacement_type
			).toFixed(2),
			tooltip: (
				<math>
	...
	useEffect(() => {
		const valuesChanged =
			...
			JSON.stringify(fraisValues) !==
				JSON.stringify(previousValuesRef.current?.frais);
	...
	return (
		<form onSubmit={handleSubmit(submitForm)}>
			...
			<div className="total-box">
				<TextItem path="frais.total_frais" tag="strong" />{" "}
